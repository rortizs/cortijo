...
//
$filaHoja = 2;
$workbook -> HeaderingExcel('Detalle_Asistencia_Mensual.xls');
$worksheet1 = &$workbook -> add_worksheet('Asistencia_Mensual');
//FORMATOS CELDAS
...
//ENCABEZADOS
$worksheet1 -> write_string(0, 0, 'Reporte Detalle Asistencia Mensual', $formato_encabezado);
$worksheet1 -> merge_cells(0, 0, 0, $lastDayMonth + 2);
...
for ($a = 0; $a < count($getEmpleados); $a++) {
	...
	$times = array();
	for ($i = 1; $i <= $lastDayMonth; $i++) {
		$getTipoMarcajes = $inventarios -> getTipoMarcajes($getEmpleados[$a]['id'], date('Y-' . $_REQUEST['month'] . '-' . addZero($i)));
		$worksheet1 -> write_string($filaHoja, (2 + $i), $getTipoMarcajes ? : '-', $formato_celda);
	}
	$filaHoja++;
}
$workbook -> close();
