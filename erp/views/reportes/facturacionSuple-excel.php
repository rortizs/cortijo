...
$existencias = $reportes->consultaFacturasSuple($_REQUEST);
//FORMATOS CELDAS
...
$worksheet1->write_string(1, 4, 'Nombre Cliente', $formato_encabezado);
$worksheet1->write_string(1, 5, 'Sucursal', $formato_encabezado);
$worksheet1->write_string(1, 6, 'Vendedor', $formato_encabezado);
$worksheet1->write_string(1, 7, 'Estatus', $formato_encabezado);
$worksheet1->write_string(1, 8, 'Fecha Anulacion', $formato_encabezado);
$worksheet1->write_string(1, 9, 'Galones', $formato_encabezado);
...
foreach ($existencias as $key => $value) {
    ...
    $worksheet1->write_string($filaHoja, 4, $value['nombreCliente'], $formato_celda);
    $worksheet1->write_string($filaHoja, 5, $value['sucursal'], $formato_celda);
    $worksheet1->write_string($filaHoja, 6, $value['vendedor'], $formato_celda);
    $worksheet1->write_string($filaHoja, 7, $value['estatus'], $formato_celda);
    $worksheet1->write_string($filaHoja, 8, $value['fechaAnulacion'], $formato_celda);
    $worksheet1->write_string($filaHoja, 9, number_format($value['cantidad'], 2), $formato_celda);
...
$worksheet1->merge_cells($filaHoja, 0, $filaHoja, 8);
$worksheet1->write_string($filaHoja, 9, number_format($total6, 2), $formato_encabezado);
