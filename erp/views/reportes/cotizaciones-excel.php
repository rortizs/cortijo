...
$filaHoja = 2;
$workbook->HeaderingExcel('Cotizaciones-' . $_REQUEST['idPuntoIngresoTxt'] . '.xls');
$worksheet1 = & $workbook->add_worksheet('Reporte Cotizacion');
$_REQUEST['idEmpresas'] = $_SESSION['idEmpresa'];
$existencias = $caja->consultarCotizaciones($_REQUEST);
//FORMATOS CELDAS
...
//ENCABEZADOS
$worksheet1->write_string(0, 0, 'Reporte de Cotizacion' . $_REQUEST['idPuntoIngresoTxt'] . '', $formato_encabezado);
...
$worksheet1->write_string(1, 9, 'No. Pedido', $formato_encabezado);
...
foreach ($existencias as $key => $value) {
    ...
    $worksheet1->write_string($filaHoja, 5, $value['observaciones'], $formato_celda);
    $worksheet1->write_string($filaHoja, 6, $value['idSucursales'], $formato_celda);
    ...
    $worksheet1->write_string($filaHoja, 9, $value['noPedido'], $formato_celda);
