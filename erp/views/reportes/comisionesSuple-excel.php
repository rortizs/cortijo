...
$filaHoja = 2;
$workbook->HeaderingExcel('reporteComisiones.xls');
$worksheet1 = & $workbook->add_worksheet('Reporte de Comisiones');
$_REQUEST['idEmpresas'] = $_SESSION['idEmpresa'];
$reporte = $reportes->reporteComisionesSuple($_REQUEST);
//exit();
//FORMATOS CELDAS
...
//ENCABEZADOS
$worksheet1->write_string(0, 0, 'Reporte Comisiones ' . strtoupper($_REQUEST['vendedor']) . ' Rango de Fechas ' . $_REQUEST['fechaInicio'] . ' al ' . $_REQUEST['fechaFin'] . '', $formato_encabezado);
...
$worksheet1->write_string(1, 11, 'Galones', $formato_encabezado);
$totalAnticipo = 0;
$totalSaldo = 0;
$totalVenta = 0;
//CONTENIDO
foreach ($reporte as $key => $value) {
    ...
    $worksheet1->write_string($filaHoja, 11, number_format($value['galonesVenta'], 2), $formato_celda);
    $filaHoja++;
}
/*
