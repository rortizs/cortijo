...
<div class="row">
    ...
    for ($a = 0; $a < count($tableStructure); $a++) {
        ...
        $requiredField = '';
        $idBox = '';
        if ($tableStructure[$a]['COLUMN_NAME'] != 'id' && $tableStructure[$a]['COLUMN_NAME'] != 'pwd' && $tableStructure[$a]['COLUMN_NAME'] != 'updated_at' && $tableStructure[$a]['COLUMN_NAME'] != 'image' && $tableStructure[$a]['COLUMN_NAME'] != 'codigoCliente' && $tableStructure[$a]['COLUMN_NAME'] != 'saldoLibros' && $tableStructure[$a]['COLUMN_NAME'] != 'saldoBanco' && $tableStructure[$a]['COLUMN_NAME'] != 'created_at' && $tableStructure[$a]['COLUMN_NAME'] != 'estado') {
            ...
            switch ($tableStructure[$a]['DATA_TYPE']) {
                case 'int':
                    ...
                    if ($_SESSION['idEmpresa'] != '') {
                        ...
                        if ($tableR == 'paises') {
                            $parametros = 'group by descripcion';
                        }
                        if ($dbS == 'pos_kasualcosmeticos') {
                            if ($tableR == 'familiaNivel2') {
                                $parametros = 'where idFamiliaNivel1=' . $getRecord['idFamiliaNivel1'] . '';
                            }
                            if ($tableR == 'familiaNivel3') {
                                $parametros = 'where idFamiliaNivel2=' . $getRecord['idFamiliaNivel2'] . '';
                            }
                        }
                    }
                    ...
                    for ($b = 0; $b < count($dataTable); $b++) {
                        switch ($tableR) {
                            ...
                            case 'clientes':
                                if ($getRecord[$tableStructure[$a]['COLUMN_NAME']] == $dataTable[$b][$selectTable[0]['COLUMN_NAME']]) {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . " selected=''>" . $dataTable[$b]['nitC'] . " - " . $dataTable[$b]['nombreC'] . "</option>";
                                } else {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . ">" . $dataTable[$b]['nitC'] . " - " . $dataTable[$b]['nombreC'] . "</option>";
                                }
                                break;
                            case 'ventas':
                                if ($getRecord[$tableStructure[$a]['COLUMN_NAME']] == $dataTable[$b][$selectTable[0]['COLUMN_NAME']]) {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . " selected=''>" . $dataTable[$b]['serie'] . " - " . $dataTable[$b]['correlativo'] . "</option>";
                                } else {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . ">" . $dataTable[$b]['serie'] . " - " . $dataTable[$b]['correlativo'] . "</option>";
                                }
                                break;
                            case 'productos':
                                if ($getRecord[$tableStructure[$a]['COLUMN_NAME']] == $dataTable[$b][$selectTable[0]['COLUMN_NAME']]) {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . " selected=''>" . $dataTable[$b]['sku'] . " - " . $dataTable[$b]['descLarga'] . "</option>";
                                } else {
                                    $editor .= "<option value=" . $dataTable[$b][$selectTable[0]['COLUMN_NAME']] . ">" . $dataTable[$b]['sku'] . " - " . $dataTable[$b]['descLarga'] . "</option>";
                                }
                                break;
                            default:
                                if ($getRecord[$tableStructure[$a]['COLUMN_NAME']] == $dataTable[$b][$selectTable[0]['COLUMN_NAME']]) {
                ...
                case 'time':
                    ...
                    break;
                case 'timestamp':
                    $editor = "<input class='form-control input-sm timestamp' type='text' id='" . $tableStructure[$a]['COLUMN_NAME'] . "' size='30' value='" . $getRecord[$tableStructure[$a]['COLUMN_NAME']] . "'/>";
                    break;
                case 'double':
            ...
            ?>
            <div class="col-xs-<?= $commentField[1] ?: 12; ?>" id="box-<?= $idBox; ?>">
...
if ($flag == 1) {
    ?>
    <button class="btn btn-primary btn-sm" onclick="saveRecord('<?= substr($campos1, 0, strlen($campos1) - 1); ?>', '<?= $table; ?>',<?= $_REQUEST['btnNew']; ?>,<?= $_REQUEST['btnUpdate']; ?>,<?= $_REQUEST['btnDelete']; ?>, '<?= $_REQUEST['modulo']; ?>', '<?= $_REQUEST['opcion']; ?>', '<?= substr($required, 0, strlen($required) - 1); ?>', '<?= substr($requiredName, 0, strlen($requiredName) - 1); ?>');">
    ...
    ?>
    <button class="btn btn-primary btn-sm" onclick="saveUpdateRecord('<?= substr($campos1, 0, strlen($campos1) - 1); ?>', '<?= $table; ?>', '<?= $_REQUEST['field']; ?>', '<?= $_REQUEST['value']; ?>',<?= $_REQUEST['btnNew']; ?>,<?= $_REQUEST['btnUpdate']; ?>,<?= $_REQUEST['btnDelete']; ?>, '<?= $_REQUEST['modulo']; ?>', '<?= $_REQUEST['opcion']; ?>', '<?= substr($required, 0, strlen($required) - 1); ?>', '<?= substr($requiredName, 0, strlen($requiredName) - 1); ?>');">
